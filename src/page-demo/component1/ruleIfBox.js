...
const COMP_DISTANCE = 80 // 组件间纵向距离
const PATH_WIDTH = 60 // 曲线宽度

export default class RuleIfBox extends Component {
  // type 1: 条件选择组件 且/或; 2: 条件配置组件; 3: 操作组件 添加条件/添加联合条件
  state = {
    data: [{
      type: 1,
      flag: '0',
  ...
  componentWillMount() {
    ...
    this.boxId = document.getElementById(id)
  }
  ...
  renderRuleCondition = () => {
    ...
    return data.map((d, i) => {
      if (d.type === 1) {
        return <RuleCondition key={`ruleCondition${d.flag}`} pos={[d.x, d.y]} delCon={() => this.delCombineCon(d, i)} {...d} /> 
      }
      return null
    })
  }
  ...
  renderRuleItem = () => {
    ...
    return data.map((d, i) => {
      if (d.type === 2) {
        return <RuleItem key={`ruleItem${d.flag}`} pos={[d.x, d.y]} delCon={() => this.delCon(d, i)} {...d} /> 
      }
      return null
    })
  }
  ...
  updateLevel = (current, type, free) => {
    ...
    const height = newData.filter(d => d.type !== 1).length * 80
    const {id} = this.props
    document.getElementById(id).style.height = `${height > 200 ? height : 200}px`
  }
  ...
  render() {
    const {id} = this.props
    return (
      <div className="rule-if-box" id={id}>
