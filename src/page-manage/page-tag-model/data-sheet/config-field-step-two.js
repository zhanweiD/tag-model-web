...
class StepTwo extends React.Component {
  ...
  render() {
    ...
    const columns = [
...

      {
        title: intl
          .get(
            'ide.src.page-manage.page-tag-model.data-sheet.config-field-step-two.fpsks5z6ei'
          )
          .d('关联的字段'),
        key: 'dataFieldName',
...

      {
        title: intl
          .get(
            'ide.src.page-manage.page-tag-model.data-sheet.config-field-step-two.714e3sh7iec'
          )
          .d('确认结果'),
        key: 'result',
        width: 190,
...

      {
        ...
        render: (v, record, index) => (
          <span>
            <a href onClick={() => this.removeItem(index, record)}>
              {intl
                .get('ide.src.page-config.workspace-config.main.i53j7u2d9hs')
                .d('移除')}
            </a>
            <Divider type="vertical" />
            <a href onClick={() => this.showEditModal(index, record)}>
              {intl
      ...
      <div>
        <Spin spinning={false}>
          <Alert
            ...
            message={
              <span className="fs12">
                {intl
                  .get(
                    'ide.src.page-manage.page-tag-model.data-sheet.config-field-step-two.vihwihdu9vk'
                  )
                  .d('选择结果：可新建标签')}
          ...
          <div className="mb16 ml2 mt24">
            <div>
              ...
              <QuestionTooltip
                tip={intl
                  .get(
                    'ide.src.page-manage.page-tag-model.data-sheet.config-field-step-two.i8ug19snd8h'
                  )
                  .d(
                    '若您不选择标签所属类目，那么标签将被放在该对象的默认类目中'
                  )}
              />
              <Checkbox
                className="ml16"
                checked={store.checkedPulish}
                onChange={this.checked}
              >
                {intl
                  .get(
                    'ide.src.page-manage.page-tag-model.data-sheet.config-field-step-two.2zish1fv063'
                  )
                  .d('生成的标签是否直接发布')}
              </Checkbox>
            </div>
          ...
          <Table
            ...
            rowSelection={{
              // fixed: true,
              selectedRowKeys: store.secondSelectedRows.map(
  ...
  @action.bound removeItem(index, record) {
    ...
    // 如果是被选中的，还需要更新选中数组
    store.secondSelectedRows = store.secondSelectedRows.filter(
      item => item.dataFieldName !== record.dataFieldName
    )
  ...
  isValueTypeName = num => {
    switch (num) {
      case 2:
        return intl.get('ide.src.common.dict.z1n81lmrcs').d('整数型')
      case 3:
        return intl.get('ide.src.common.dict.o41130chdhe').d('小数型')
      case 4:
        return intl.get('ide.src.common.dict.vdvia4exsvj').d('文本型')
      default:
        return intl.get('ide.src.common.dict.k7axbetkeh9').d('日期型')
    }
  }
